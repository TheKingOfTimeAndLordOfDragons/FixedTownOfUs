...
namespace TownOfUs.NeutralRoles.PhantomMod
{
    ...
    public class SetPhantom
    {
        ...
        public static void ExileControllerPostfix(ExileController __instance)
        {
            ...
            var doomRole = Role.AllRoles.FirstOrDefault(x => x.RoleType == RoleEnum.Doomsayer && Doomsayer.WonByGuessing && ((Doomsayer)x).Player == WillBePhantom);
            if (doomRole != null) return;
            var exeRole = Role.AllRoles.FirstOrDefault(x => x.RoleType == RoleEnum.Executioner && Executioner.TargetVotedOut && ((Executioner)x).Player == WillBePhantom);
            if (exeRole != null) return;
            var jestRole = Role.AllRoles.FirstOrDefault(x => x.RoleType == RoleEnum.Jester && Jester.VotedOut && ((Jester)x).Player == WillBePhantom);
