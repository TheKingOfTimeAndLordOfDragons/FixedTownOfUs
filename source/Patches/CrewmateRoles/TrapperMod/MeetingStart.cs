...
namespace TownOfUs.CrewmateRoles.TrapperMod
{
    ...
    public class MeetingStart
    {
        public static void Postfix(MeetingHud __instance)
        {
            ...
            {
                DestroyableSingleton<HudManager>.Instance.Chat.AddChat(PlayerControl.LocalPlayer, Language.GetString("roles.crewmate.trapper.trap.info.noplayers"));
            }
            else if (trapperRole.trappedPlayers.Count < CustomGameOptions.MinAmountOfPlayersInTrap)
            {
                DestroyableSingleton<HudManager>.Instance.Chat.AddChat(PlayerControl.LocalPlayer, Language.GetString("roles.crewmate.trapper.trap.info.notenough"));
            }
            else
            {
                string message = Language.GetString("roles.crewmate.trapper.trap.info.caught") + "\n";
